import { apiCall, showNotification, isAuthenticated, checkLoginStatus, logout } from './utils.js';
import { fetchKeywords, fetchSkills, fetchStats, fetchProjects } from './api.js';
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            <small class="text-muted">Desenvolvido por: ${
                                project.developers.map(dev => dev.name).join(', ')
                            }</small>
                        </div>
                        <a href="${project.link}" class="btn btn-primary animated-button" target="_blank">
                            Ver Projeto
                        </a>
                    </div>
                </div>
...
function createDigitalRain() {
    ...
    for (let i = 0; i < columnCount; i++) {
        ...
        let rainString = '';
        for (let j = 0; j < 20; j++) {
            rainString += characters.charAt(Math.floor(Math.random() * characters.length));
        }
